...
def run_benchmark(filename, ncells_per_dir : tuple[int, int, int] = (10,10,10), dt=0.001, n_steps=5):
    global system
    system = NBodySystem(filename, ncells_per_dir=ncells_per_dir)

    # Échauffement hors mesure pour exclure le coût de compilation JIT.
    system.update_positions(dt)
    ...
    print(f"Benchmark update_positions sur {filename}")
    ...
    print(f"Average update time: {avg_ms:.3f} ms")
    return avg_ms
...
if "--benchmark" in sys.argv:
    bench_idx = sys.argv.index("--benchmark")
    benchmark_steps = 5
    if bench_idx + 1 < len(sys.argv):
        benchmark_steps = int(sys.argv[bench_idx + 1])

if benchmark_steps > 0:
    print(f"Benchmark de {filename} avec dt = {dt} et grille {n_cells_per_dir}")
    run_benchmark(filename, ncells_per_dir=n_cells_per_dir, dt=dt, n_steps=benchmark_steps)
else:
