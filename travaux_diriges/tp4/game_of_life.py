...
# Para probar si un proceso le paso algo al otro necesitamos usar la function mpi.probre
# Siempre utilizamos el proceso 0 para el afichage y los otros para los calculos
...
if __name__ == '__main__':
    ...
    if samples > 0:
        avg_compute_ms = (sum_compute / samples) * 1e3
        avg_draw_ms = (sum_draw / samples) * 1e3
        avg_frame_ms = (sum_frame / samples) * 1e3
        fps = 1.0 / (sum_frame / samples) if (sum_frame / samples) > 0 else 0.0
        changed_per_frame = sum_changed / samples
    else:
        avg_compute_ms = avg_draw_ms = avg_frame_ms = fps = changed_per_frame = 0.0

    print("\n===== Secuencial : métricas =====")
    print(f"Grid: {grid.dimensions[0]}x{grid.dimensions[1]} | frames(samples)={samples}")
    print(
        f"Seq: avg compute={avg_compute_ms:.3f} ms | avg draw={avg_draw_ms:.3f} ms | "
        f"avg frame={avg_frame_ms:.3f} ms | FPS≈{fps:.2f} | changed/frame≈{changed_per_frame:.2f}"
    )
    pg.quit()
